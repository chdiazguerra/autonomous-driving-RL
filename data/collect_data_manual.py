...
class World(object):
    def __init__(self, carla_world, hud, args, weather, traffic_manager):
        #ADDED
        self.args = args
    ...
    def restart(self):
        ...
        blueprint = self.world.get_blueprint_library().find('vehicle.audi.a2') #CHANGED
        ...
        camera_path = os.path.join(self.args.out_folder, 'camera')
        depth_path = os.path.join(self.args.out_folder, 'depth')
        semantic_path = os.path.join(self.args.out_folder, 'semantic')
        os.makedirs(camera_path, exist_ok=True)
        os.makedirs(depth_path, exist_ok=True)
        os.makedirs(semantic_path, exist_ok=True)

        blueprint_library = self.world.get_blueprint_library()
        for _ in range(self.args.nb_vehicles):
            while True:
                spawn_point = random.choice(self.map.get_spawn_points())
                vehicle = self.world.try_spawn_actor(random.choice(blueprint_library.filter('vehicle.*')),
                                                spawn_point)
        ...
        obstacle_bp.set_attribute('sensor_tick', str(self.args.tick))
        obstacle_transform = carla.Transform()
        obstacle = self.world.spawn_actor(obstacle_bp, obstacle_transform, attach_to=self.player)
        obstacle.listen(lambda data: sensor_callback(data, self.sensor_queue, SENSOR_TYPE['OBSTACLE']))
        self.sensors.append(obstacle)
        ##################
        if self.sync:
            self.world.tick()
        else:
...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        if keys[K_UP] or keys[K_w]:
            if not self._ackermann_enabled:
                self._control.throttle = min(self._control.throttle + 0.01, 0.8) #CHANGED
            else:
                self._ackermann_control.speed += round(milliseconds * 0.005, 2) * self._ackermann_reverse
        else:
            if not self._ackermann_enabled:
                self._control.throttle = max(self._control.throttle - 0.01, 0.0) #CHANGED
...
def game_loop(args):
    ...
    try:
        ...
        world = World(sim_world, hud, args, weather_presets[args.weather], traffic_manager)
        ...
        for _ in range(args.nb_frames):
            ...
            #ADDED
            car_loc = world.player.get_location()
            current_wp = world.map.get_waypoint(car_loc)
            
            distance = distance_from_center(current_wp, current_wp.next(0.001)[0], car_loc)

            ego_yaw = world.player.get_transform().rotation.yaw
    ...
    #ADDED
        ...
        #ADDED
        sensor_data = []
        while not world.sensor_queue.empty():
            data = world.sensor_queue.get()
